...
Web search is automatically routed through Gemini CLI via a PreToolUse hook. When you call WebSearch, the hook:

1. Intercepts the query
2. Runs Gemini CLI search (with 1hr caching)
3. Returns grounded Google results

Just use WebSearch normally - the hook handles the rest. No special action needed.

**Cache Management:**
- `/search-stats` - View cache statistics
